...
class Block {
    ...
    constructor(x, y) {
        if (x < 0 || y < 0) throw new TetrisError('Bad constructor use of Block');
        this.x = x;
        this.y = y;
    }
...
class BlockGroup {
    ...
    rotate() {
        ...
        if (Math.max(...this.blockArray.map(b => b.getX() + 1)) >= FIELD_WIDTH ||
...
interfaceButton.style.top =
    PIXEL_SIZE * FIELD_HEIGHT + interfaceButton.clientTop + 'px';
...
document.addEventListener('DOMContentLoaded', () => {
    rightBtn.style.left = leftBtn.offsetLeft + leftBtn.clientWidth + PIXEL_SIZE + 'px';
    downBtn.style.left = rightBtn.offsetLeft + rightBtn.clientWidth + PIXEL_SIZE + 'px';
    buttons.style.top = gameField.offsetTop + gameField.clientHeight + PIXEL_SIZE * 2 + 'px';
});